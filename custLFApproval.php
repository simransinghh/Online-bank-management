...
$dstatus = "";
   if($cntt > 0){
     ...
     while ($row = mysql_fetch_array($qryForm)){
        $dEmail=$row['email'];
        $dfname = $row['fname'];
        $dlname = $row['lname'];
        $dage = $row['age'];
        $ddob = $row['dob'];
        $dmincome = $row['minincome'];
        $dlaneed = $row['laneed'];
...
<input type="text" placeholder="Enter Email" name="email" value="<?PHP echo $dEmail; ?>"  <?PHP if($flagCK == "YES"){
?> disabled <?PHP } ?> required>
<hr>
...
<select id="purpose" name="purpose" size="1.5"><?PHP if($flagCK == "YES"){ ?> disabled <?PHP } ?>>
  <optgroup>
<option value="Housing Loan;font-size:20px;" <?php if($dpurpose == "" || $dpurpose == "Housing Loan"){ echo "selected"; } ?>>Housing Loan</option>
...
<div class="buttona">
	<button type = "submit" style="width: 0px; height: 0px; font-size: 24px; font-family: sans-serif;
  font-weight: bolder;"  name="statusAct" value="Accept">ACCEPT</button>
</div>
<div class="buttonb"> 
	<button type="submit"  style="width: 0.0%;height: 0px; font-size: 24px; font-family: sans-serif;
  font-weight: bolder;" name="statusAct" value="Reject">REJECT</button>
</div>
<div class="buttonc">
	<button type="button" style="width: 0.00%; height: 0px; font-size: 24px; font-family: sans-serif;
  font-weight: bolder;" onclick="window.location.href='custList.php?aid=<?php echo $aID;?>&uname=<?php echo $uName;?>'">BACK</button>
</div>
</div>
</form>
